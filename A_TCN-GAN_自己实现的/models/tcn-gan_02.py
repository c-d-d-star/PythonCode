...
class TemporalConvNet(nn.Module):
  def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2,max_length=16, attention=True):
...
class AttentionBlock(nn.Module):
  ...
  Arguments:
      ...
      一种类似于Vaswani等人(2017)的注意机制
      AttentionBlock的输入是' BxTxD '，其中' B '是输入
      小批量尺寸，' T '是序列的长度' D '是尺寸
      每个特性。
      AttentionBlock的输出是' BxTx(D+V) '，其中' V '是对象的大小
      注意值。
      参数:
      Dims (int):每个元素的维度(或通道)数
      输入序列
      K_size (int):注意键的大小
      V_size (int):注意值的大小
      Seq_len (int):输入和的长度
...
class TCN(nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout,attention=False):
...
parser.add_argument('--nz', type=int, default=60, help='dimensionality of the latent vector z')
...
#         #     writer.add_histogram("GeneratorGradients/{}".format(name), param.grad, niter)
        print('[%d/%d][%d/%d] Loss_D: %.7f Loss_G: %.7f D(x) : %.4f D(G(z)): %.7f / %.7f '% (epoch, opt.epochs, i, len(train_loader),errD.item(), errG.item(), D_x, D_G_z1, D_G_z2,), end='\n')
#         # #### Report metrics #####
        #
        #
        #
